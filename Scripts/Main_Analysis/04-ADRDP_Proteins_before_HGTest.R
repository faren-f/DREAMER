...
ADRDP_proteins_before_HGTest = list()
N = c()
for(s in ADRs){
  ...
  actualScore = Scores[[s]][["Actual_protein_scores"]]
  ...
  ADRDP_proteins_before_HGTest[[s]] = rownames(Null_score_ADR_Protein)[which(qval_ADR_protein<0.05 & qval_DP_protein<0.05)]
...
saveRDS(ADRDP_proteins_before_HGTest, "Data/ADRDP_Proteins_before_HGTest.rds")
