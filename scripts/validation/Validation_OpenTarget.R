...
###########################################
## symptom_gene OpenTarget
symptom_gene = readRDS("data/other_resources/OpenTarget_Overal_direct.rds")
symptom_gene = readRDS("data/other_resources/OpenTarget_Overal_Indirect.rds")

symptom_gene = symptom_gene[,c(6,5,3)]
colnames(symptom_gene)[2] = "Gene_Symbol"
I = intersect(symptom_gene$Gene_Symbol, PPI_geneNames_ids[,1])
symptom_gene = symptom_gene[symptom_gene$Gene_Symbol %in% I,]
symptom_gene = unique(symptom_gene)
...
####################################
#How many ADRs are common between OpenTarget and our network
...
I = intersect(unique(symptom_gene$umls_id), UMLS_meddra$umls_id)
symptom_gene = symptom_gene[symptom_gene$umls_id %in% I,]
...
################################################################################
####Overlap SOA_ADR-Proteins with OpenTarget
...
######################################
####Overlap Diffusion DP-Protein with OpenTarget
...
######################################
####Overlap Diffusion ADR-Protein with OpenTarget
...
ggplot(data, aes(x=methods, y=values, fill=methods)) + 
  ...
  theme_classic() + scale_y_continuous(n.breaks = 5)+
